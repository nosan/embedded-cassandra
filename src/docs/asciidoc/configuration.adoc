...
`CassandraDirectoryProvider` provides a path to Cassandra directory based on a version. Default implementation of this
interface is `WebCassandraDirectoryProvider`.
...
If the Cassandra directory have already extracted then it will be used, skipping downloading and extracting steps.
...
It is useful for unit testing. Highly likely, you would not want to start and stop the Cassandra instance
between tests.

== Startup Timeout

A duration of time to wait, while Cassandra is being ready to accept connections.
...
`cassandra-topology.properties`, certificates or something else, `addResource` can be used.
