 ...
 * @author Dmytro Nosan
 */
class ConfigurationFileCustomizerTests {

	@Test
	void customize(@TempDir Path temporaryFolder) throws Exception {
		Path conf = temporaryFolder.resolve("conf");
		Files.createDirectories(conf);
		ConfigurationFileCustomizer customizer = new ConfigurationFileCustomizer(
		...
		try (InputStream inputStream = getClass().getResourceAsStream("/cassandra.yaml")) {
			assertThat(conf.resolve("cassandra.yaml")).hasBinaryContent(IOUtils.toByteArray(inputStream));
		}
