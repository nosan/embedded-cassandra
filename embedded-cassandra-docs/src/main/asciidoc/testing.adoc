...
`@CassandraScripts` annotation is used to annotate a test class to execute CQL scripts against Cassandra.
...
=== Configure Cql Scripts Executor

CQL Scripts from `@CassandraScripts` annotation are executed by `CassandraScriptsExecutor`.
