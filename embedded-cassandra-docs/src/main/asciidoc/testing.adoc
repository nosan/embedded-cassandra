...
By default, `CqlScriptsExecutor` implementation is detected based on the classpath, e.g. if `com.datastax.driver.core.Cluster`
...
therefore, `CqlScriptsExecutor` could be registered as a `@Bean` to control scripts execution.

[source,java,indent=0]
----
include::{test-sources}/examples/spring/configuration/cql/CassandraCustomScriptExecutorSpringTests.java[tag=source]
----
