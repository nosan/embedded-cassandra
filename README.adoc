...
should use `TestCassandra` from `embedded-cassandra-test` module instead.
...
```
TIP: You can declare `CassandraFactory` and `ClusterFactory` beans to take control of the `Cassandra` instance's. Also,
it is possible to use `@LocalCassandra` annotation that allows to configure and register `CassandraFactory` bean.
